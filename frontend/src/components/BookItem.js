...
export const BookItem = ({ book, source }) => {
    const {user} = useAuth();
    const {newBook, rating} = useSelector(state => state.review);
    ...
    return (
        <div id="tbodyTr">
            ...
            <div>{typeof book.id !== "undefined" ? (<button type="button"><span>&#43;</span> View</button>) : (Object.keys(newBook).length > 0 ? <button type="button"><span>&#43;</span> View</button> : <button type="button" onClick={() => addToBookShelf(bookDetails)}><span>&#43;</span> Add To BookShelf</button>)}</div>
        </div>
    );
