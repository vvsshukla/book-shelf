...
export const Reader = ({ reader, userType }) => {
    ...
    const { receiverId } = useSelector(state => state.friend);
    ...
    return (
        <div className="reader">
            ...
            {userType === 'receiver' ? (<><button type="button" className="frActions fa fa-check" onClick={() => friendRequestAction(reader._id, 'accepted')}>Accept</button><button type="button" className="frActions fa fa-ban" onClick={() => friendRequestAction(reader._id, 'rejected')}>Reject</button></>) : (reader._id === receiverId ? <span>Request sent.</span> : <button onClick={() => add(reader._id)} className="addFrientBtn" title="Send Friend Request">&#43;</button>)}
        </div>
    );
}